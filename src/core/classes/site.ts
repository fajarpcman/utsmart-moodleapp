...
const ALLOWED_LOGGEDOUT_WS = ["core_user_remove_user_device"];
...
export class CoreSite {
  ...
  protected configTable: AsyncInstance<
    CoreDatabaseTable<CoreSiteConfigDBRecord, "name">
  >;
  protected lastViewedTable: AsyncInstance<
    CoreDatabaseTable<CoreSiteLastViewedDBRecord, "component" | "id">
  >;
  protected cleanUnicode = false;
  protected lastAutoLogin = 0;
  protected offlineDisabled = false;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  protected ongoingRequests: Record<
    string,
    Record<OngoingRequestType, WSObservable<any> | undefined>
  > = {};
  ...
  constructor(
    ...
    this.cacheTable = asyncInstance(() =>
      CoreSites.getSiteTable(WS_CACHE_TABLE, {
        siteId: this.getId(),
        database: this.getDb(),
    ...
    this.configTable = asyncInstance(() =>
      CoreSites.getSiteTable(CONFIG_TABLE, {
        siteId: this.getId(),
        database: this.getDb(),
    ...
    this.lastViewedTable = asyncInstance(() =>
      CoreSites.getSiteTable(LAST_VIEWED_TABLE, {
        siteId: this.getId(),
        database: this.getDb(),
        config: { cachingStrategy: CoreDatabaseCachingStrategy.Eager },
  ...
  setConfig(config: CoreSiteConfig): void {
    if (config) {
      config.tool_mobile_disabledfeatures = CoreTextUtils.treatDisabledFeatures(
        config.tool_mobile_disabledfeatures
      );
    }
  ...
  canAccessMyFiles(): boolean {
    ...
    return !!(
      info &&
      (info.usercanmanageownfiles === undefined || info.usercanmanageownfiles)
    );
  }
  ...
  canUseAdvancedFeature(
    ...
    const feature = info.advancedfeatures.find(
  ...
  requestObservable<T = unknown>(
    ...
    const observable = this.performRequest<T>(
      method,
      data,
      preSets,
      wsPreSets
   ...
   */
  protected getOngoingRequest<T = unknown>(
    cacheId: string,
    preSets: CoreSiteWSPreSets
  ): WSObservable<T> | undefined {
    if (preSets.updateInBackground) {
      return this.ongoingRequests[cacheId]?.[
        OngoingRequestType.UPDATE_IN_BACKGROUND
      ];
   ...
   */
  protected setOngoingRequest<T = unknown>(
    cacheId: string,
    preSets: CoreSiteWSPreSets,
    request: WSObservable<T>
  ): void {
    this.ongoingRequests[cacheId] = this.ongoingRequests[cacheId] ?? {};

    if (preSets.updateInBackground) {
      this.ongoingRequests[cacheId][OngoingRequestType.UPDATE_IN_BACKGROUND] =
        request;
    } else {
   ...
   */
  protected clearOngoingRequest<T = unknown>(
    cacheId: string,
    preSets: CoreSiteWSPreSets,
    request: WSObservable<T>
  ): void {
        ...
        if (
          ...
          setTimeout(async () => {
            try {
              ...
              const newData = await this.getFromWS<T>(
                method,
                data,
                preSets,
                wsPreSets
  ...
  protected async getFromWS<T = unknown>(
    ...
    try {
      const response = await this.callOrEnqueueWS<T>(
        method,
        data,
        preSets,
        wsPreSets
      ...
      if (CoreUtils.isExpiredTokenError(error)) {
        ...
        error.errorcode === "userdeleted" ||
        error.errorcode === "wsaccessuserdeleted"
      ) {
        ...
        error.message = Translate.instant(
          "core.login.sitepolicynotagreederror"
        );

        throw new CoreWSError(error);
      } else if (
        error.errorcode === "dmlwriteexception" &&
        CoreTextUtils.hasUnicodeData(data)
      ) {
        ...
        error.exception === "required_capability_exception" ||
        error.errorcode === "nopermission" ||
        ...
        preSets.cacheErrors &&
        preSets.cacheErrors.indexOf(error.errorcode) != -1
      ) {
        // Save the error instead of deleting the cache entry so the same content is displayed in offline.
        this.saveToCache(method, data, error, preSets);
        ...
        preSets.emergencyCache !== undefined &&
        !preSets.emergencyCache
      ) {
        this.logger.debug(
          `WS call '${method}' failed. Emergency cache is forbidden, rejecting.`
        );
      ...
      this.logger.debug(
        `WS call '${method}' failed. Trying to use the emergency cache.`
      );
      preSets = {
        ...preSets,
        omitExpires: true,
      ...
      try {
        const cachedData = await this.getFromCache<T>(
          method,
          data,
          preSets,
          true
        ...
        if (
          typeof cachedData.response === "object" &&
          cachedData.response !== null &&
          (("exception" in cachedData.response &&
            cachedData.response.exception !== undefined) ||
            ("errorcode" in cachedData.response &&
              cachedData.response.errorcode !== undefined))
        ) {
  ...
  protected async callOrEnqueueWS<T = unknown>(
    ...
    data = {
      ...data,
      moodlewssettinglang: (
        preSets.lang ?? (await CoreLang.getCurrentLanguage())
      ).replace("-", "_"),
    };

    try {
      return await this.callOrEnqueueRequest<T>(
        method,
        data,
        preSets,
        wsPreSets
      ...
      if (
        ...
        method === "core_webservice_get_site_info"
      ) {
  ...
  protected callOrEnqueueRequest<T = unknown>(
    ...
    if (
      preSets.skipQueue ||
      !this.wsAvailable("tool_mobile_call_external_functions")
    ) {
    ...
    if (preSets.reusePending) {
      const request = this.requestQueue.find(
        (request) => request.cacheId == cacheId
      );
      if (request) {
  ...
  protected async processRequestQueue(): Promise<void> {
    ...
    if (requests.length == 1 && !CoreSite.REQUEST_QUEUE_FORCE_WS) {
      ...
      try {
        const data = await CoreWS.call(
          requests[0].method,
          requests[0].data,
          requests[0].wsPreSets
        );
      ...
      requests.forEach((request, i) => {
        ...
        if (!response) {
          ...
          request.deferred.reject(
            CoreTextUtils.parseJSON(response.exception || "")
          );
        } else {
          let responseData = response.data
            ? CoreTextUtils.parseJSON(response.data)
            : {};
          // Match the behaviour of CoreWSProvider.call when no response is expected.
          const responseExpected =
            wsPresets.responseExpected === undefined ||
            wsPresets.responseExpected;
          if (
            !responseExpected &&
  ...
  protected getCacheId(method: string, data: any): string {
    return <string>(
      Md5.hashAsciiStr(method + ":" + CoreUtils.sortAndStringify(data))
    );
  }
  ...
  protected async getFromCache<T = unknown>(
    ...
    if (
      preSets.getCacheUsingCacheKey ||
      (emergency && preSets.getEmergencyCacheUsingCacheKey)
    ) {
    ...
    const forceCache =
      preSets.omitExpires || preSets.forceOffline || !CoreNetwork.isOnline();
    ...
    if (!forceCache) {
      ...
      if (
        ...
        const extendedTime =
          ...
          (CoreConstants.CONFIG.callWSInBackgroundExpirationTime ??
            CoreConstants.SECONDS_WEEK * 1000);

        if (now > extendedTime) {
          this.logger.debug(
            "Cached element found, but it is expired even for call WS in background."
          );
    ...
    if (entry.data !== undefined) {
      if (!expirationTime) {
        this.logger.info(
          `Cached element found, id: ${id}. Expiration time ignored.`
        );
      } else {
        const expires = (expirationTime - now) / 1000;
        this.logger.info(
          `Cached element found, id: ${id}. Expires in expires in ${expires} seconds`
        );
      }
    ...
    return this.cacheTable.reduce(
      ...
      },
      {
        ...
        js: (record) =>
          ...
          (params.length === 1 || record.componentId === componentId),
      }
    );
  }
  ...
  protected async saveToCache(
    ...
    if (preSets.uniqueCacheKey) {
      ...
      await CoreUtils.ignoreErrors(
        this.deleteFromCache(method, data, preSets, true)
      );
    }
   ...
   * @returns Promise resolved when the entries are deleted.
   */
  async deleteComponentFromCache(
    component: string,
    componentId?: number
  ): Promise<void> {
    if (!component) {
  ...
  async invalidateWsCacheForKeyStartingWith(key: string): Promise<void> {
    ...
    await this.cacheTable.updateWhere(
      { expirationTime: 0 },
      {
        sql: "key LIKE ?",
        sqlParams: [key + "%"],
  ...
  fixPluginfileURL(url: string): string {
    const accessKey =
      this.tokenPluginFileWorks || this.tokenPluginFileWorks === undefined
        ? this.infos && this.infos.userprivateaccesskey
        : undefined;

    return CoreUrlUtils.fixPluginfileURL(
      url,
      this.token || "",
      this.siteUrl,
      accessKey
  ...
  containsUrl(url?: string): boolean {
    ...
    const siteUrl = CoreTextUtils.addEndingSlash(
      CoreUrlUtils.removeProtocolAndWWW(this.siteUrl)
    );
  ...
  async getPublicConfig(
    options: { readingStrategy?: CoreSitesReadingStrategy } = {}
  ): Promise<CoreSitePublicConfigResponse> {
    if (!this.db) {
    ...
    // Check for an ongoing identical request.
    const ongoingRequest = this.getOngoingRequest<CoreSitePublicConfigResponse>(
      cacheId,
      cachePreSets
    );
    if (ongoingRequest) {
    ...
    this.getFromCache<CoreSitePublicConfigResponse>(
      method,
      {},
      cachePreSets,
      false
      ...
      .catch(async () => {
        ...
        try {
          ...
          try {
            const cachedData =
              await this.getFromCache<CoreSitePublicConfigResponse>(
                method,
                {},
                cachePreSets,
                true
  ...
  protected async requestPublicConfig(): Promise<CoreSitePublicConfigResponse> {
    ...
    try {
      config = await CoreWS.callAjax<CoreSitePublicConfigResponse>(
      ...
      if (
        !error ||
        error.errorcode !== "codingerror" ||
        (this.getInfo() && !this.isVersionGreaterEqualThan("3.8"))
      ) {
        throw error;
      }
  ...
  async openInAppWithAutoLogin(
    ...
    const iabInstance = <InAppBrowserObject>(
      await this.openWithAutoLogin(true, url, options, alertMessage)
    );
  ...
  getConfig(
    name?: string,
    ignoreCache?: boolean
  ): Promise<string | CoreSiteConfig> {
    return firstValueFrom(
      this.getConfigObservable(
        <string>name,
  ...
  isFeatureDisabled(name: string): boolean {
    const disabledFeatures = this.getStoredConfig(
    ...
    const regEx = new RegExp(
      "(,|^)" + CoreTextUtils.escapeForRegex(name) + "(,|$)",
      "g"
    );
      ...
      for (let i = 0; i < versions.length; i++) {
        ...
        if (i == versions.length - 1) {
          ...
          if (
            ...
            siteVersion < this.getNextMajorVersionNumber(versions[i])
          ) {
            return true;
          }
        }
  ...
  async getAutoLoginUrl(
    ...
    if (this.lastAutoLogin > 0) {
      const timeBetweenRequests = await CoreUtils.ignoreErrors(
        ...
        CoreConstants.SECONDS_MINUTE * 0.17
      );

      if (
        CoreTimeUtils.timestamp() - this.lastAutoLogin <
        timeBetweenRequests
      ) {
    ...
    try {
      ...
      const data = await this.write<CoreSiteAutologinKeyResult>(
   ...
   */
  protected getMajorAndMinor(
    version: string
  ): { major: string; minor: number } | false {
   ...
   */
  async getLocalSiteConfig<T extends number | string>(
    name: string,
    defaultValue?: T
  ): Promise<T> {
    try {
  ...
  getExpirationDelay(updateFrequency?: number): number {
    ...
    let expirationDelay =
      this.UPDATE_FREQUENCIES[updateFrequency] ||
  ...
  checkTokenPluginFile(url: string): Promise<boolean> {
    if (
      !CoreUrlUtils.canUseTokenPluginFile(
        url,
        this.siteUrl,
        this.infos && this.infos.userprivateaccesskey
      )
  ...
  isSitePluginFileUrl(url: string): boolean {
    const isPluginFileUrl =
      CoreUrlUtils.isPluginFileUrl(url) ||
      CoreUrlUtils.isTokenPluginFileUrl(url);
    if (!isPluginFileUrl) {
      return false;
    }
   ...
   * @returns Promise resolved when done.
   */
  async deleteLastViewed(
    conditions?: Partial<CoreSiteLastViewedDBRecord>
  ): Promise<void> {
  ...
  async getLastViewed(
    component: string,
    id: number
  ): Promise<CoreSiteLastViewedDBRecord | undefined> {
    try {
  ...
  async getComponentLastViewed(
    ...
    try {
      ...
      return await this.lastViewedTable.getManyWhere({
        ...
        js: (record) =>
          record.component === component && ids.includes(record.id),
      });
    } catch {
      // Not found.
    }
...
export function chainRequests<T, O extends ObservableInput<any>>(
  ...
  return (source: WSObservable<T>) =>
    new Observable<{ data: T; readingStrategy?: CoreSitesReadingStrategy }>(
      (subscriber) => {
        let firstValue = true;
        ...
        return source.subscribe({
          next: async (value) => {
            if (
              readingStrategy !==
              CoreSitesReadingStrategy.STALE_WHILE_REVALIDATE
            ) {
